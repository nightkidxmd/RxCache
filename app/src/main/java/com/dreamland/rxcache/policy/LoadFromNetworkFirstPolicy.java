...
public class LoadFromNetworkFirstPolicy<T> implements ILoaderPolicy<T> {
    @Override
    public Observable<T> load(Context context,String url, Class<T> clazz, ICacheLoader<T> cacheLoader){
        ...
        return cacheLoader.loadFromNetwork(Observable.just(tuple3))
                .switchIfEmpty(cacheLoader.loadFromMemory(Observable.just(tuple3)))
